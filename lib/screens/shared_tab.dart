...
import '../screens/cup_card_screen.dart';
...
class SharedTab extends ConsumerWidget {
                      ...
                      if (cup.photoPaths.isNotEmpty) ...[
                        SizedBox(
                          ...
                          child: ListView.builder(
                            ...
                            itemCount: cup.photoPaths.length,
                            itemBuilder: (context, index) {
                              return Padding(
  ...
  void _showExistingBagsDialog(BuildContext context, WidgetRef ref, dynamic sharedCup) {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Select Active Bag'),
        content: SizedBox(
  ...
  void _copyToNewBag(BuildContext context, WidgetRef ref, dynamic sharedCup, dynamic sourceBag) {
    final cup = sharedCup.cupData;

    // Create initial bag from shared bag data, or create a default bag
    final initialBag = sourceBag != null
        ? CoffeeBag(
            id: '', // Will be generated by the form
            userId: '', // Will be set by the form
            customTitle: sourceBag.customTitle,
            coffeeName: sourceBag.coffeeName,
            roaster: sourceBag.roaster,
        ...
        : CoffeeBag(
            id: '', // Will be generated by the form
            userId: '', // Will be set by the form
            customTitle: 'Shared: ${cup.brewType}',
            coffeeName: cup.brewType,
            roaster: 'Unknown Roaster',
            datePurchased: DateTime.now(),
          );

    // Create initial cup with shared recipe data
    final initialCup = Cup(
      id: '', // Will be generated by the form
      bagId: '', // Will be set by the form
      userId: '', // Will be set by the form
    ...
    // Navigate to cup card screen with prepopulated data
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => CupCardScreen(
          isNewBag: true,
          initialCup: initialCup,
